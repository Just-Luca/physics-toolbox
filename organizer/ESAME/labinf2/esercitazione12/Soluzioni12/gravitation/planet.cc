...
void planet::DoStep(planet* pl1, planet* pl2, double dt)
{
  ...
  versore   = GetAccDirection(pl1);	
  fatt_molt = CONST_G*(pl1->mass_p)/pow(GetDistance(pl1),2);
	
  vett2d a1 = versore.scalar_prod(fatt_molt);
	
  versore   = GetAccDirection(pl2);
  fatt_molt = CONST_G*(pl2->mass_p)/pow(GetDistance(pl2),2);
