...
istogramma::istogramma (const int& nBin, const double& min, const double& max)
  ...
  , sum2_p       (0)
{
  // Azzera gli elementi dell'istogramma
...
istogramma istogramma::operator+ (const istogramma& histo)
{
  ...
  if (nBin_p != histo.nBin_p || min_p != histo.min_p || max_p != histo.max_p)
    {
      std::cout << "[istogramma::operator+]\tErrore: i due istogrammi";
      std::cout << " hanno un binning differente" << std::endl;
      return SumHisto;
    }
...
{
  if (entries_p > 1) return sqrt(sum2_p/entries_p - (sum_p/entries_p)*(sum_p/entries_p) * entries_p / (entries_p - 1)); // Unbiased estimator
  return 0;
}
...
void istogramma::Print () const
{
  ...
  for (int i = 0; i < nBin_p; i++)
    {
      std::cout << std::fixed << std::setw(8) << std::setprecision(2)
                << min_p + i * step_p <<"|";
      int freq = 0;
      if (max != 0) freq = int(scale * binContent_p[i] / max);
  ...
  std::cout << "        | X random variable" << std::endl;
  std::cout << "        V\n" << std::endl;
}
