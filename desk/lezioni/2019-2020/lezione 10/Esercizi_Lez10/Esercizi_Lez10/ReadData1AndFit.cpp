...
int main (int argc, char** argv)
{
  ...
  // ##############################
  TF1* myFun = new TF1("myFun", sum, xMin, xMax, nPar);
  ...
  // ###############################
  TCanvas* myCanv3 = new TCanvas("myCanv3","myCanv3",0,0,700,500);
  TH1D* h3         = new TH1D("h3","Standardized residuals plot", nBins, xMin, xMax);
  for (unsigned int i = 0; i < nBins; i++)
    h3->SetBinContent(i+1,(h1->GetBinContent(i+1) - myFun->Eval(h1->GetBinCenter(i+1))) / h1->GetBinError(i+1));
  h3->GetXaxis()->SetTitle("Variable x");
  h3->GetYaxis()->SetTitle("Standardized residuals");
