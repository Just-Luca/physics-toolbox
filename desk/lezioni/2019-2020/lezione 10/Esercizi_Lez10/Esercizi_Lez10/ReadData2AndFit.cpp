...
int main(int argc, char** argv)
{
  ...
  myHisto->GetXaxis()->SetTitle("Variable x");
  myHisto->GetYaxis()->SetTitle("Counts");
  ...
  TH1D* h2         = new TH1D("h2","Standardized residuals plot", nBins, xMin, xMax);
  for (unsigned int i = 0; i < nBins; i++)
    h2->SetBinContent(i+1,(myHisto->GetBinContent(i+1) - myFun->Eval(myHisto->GetBinCenter(i+1))) / myHisto->GetBinError(i+1));
  h2->GetXaxis()->SetTitle("Variable x");
  h2->GetYaxis()->SetTitle("Standardized residuals");
