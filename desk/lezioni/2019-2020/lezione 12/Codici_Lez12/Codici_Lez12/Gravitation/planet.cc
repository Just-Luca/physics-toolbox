...
void planet::DoStep(planet& pl1, planet& pl2, double dt)
{
  ...
  fatt_molt = CONST_G * pl1.mass_p / (planet::GetDistance(pl1) * planet::GetDistance(pl1));

  vett2d a1 = versore.scalar_prod(fatt_molt);

  versore   = planet::GetAccDirection(pl2);
  fatt_molt = CONST_G * pl2.mass_p / (planet::GetDistance(pl2) * planet::GetDistance(pl2));
  vett2d a2 = versore.scalar_prod(fatt_molt);

  vett2d a_tot = a1 + a2;

  // Variazione vettore velocita`
