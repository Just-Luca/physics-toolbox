...
void manipHisto (interfacciaIstogramma* histo, const double min, const double max, const int N)
{
  ...
  for (int i = 0; i < N; i++)
    {
      random = Statistics::rand_CLT (min, max, estrazioni);
      histo->Fill(random);
    }
...
int main(int argc, char** argv)
{
  ...
  if (argc != 2)
    {
      std::cout << "Errore! Il numero di parametri non e` corretto: ./testOOP [1=istogramma; 2=TH1D (composition); 3=TH1D (inheritance)]" << std::endl;
      return 1;
    }
  ...
  TH1D* histoTH1D = dynamic_cast<TH1D*>(histo);
  if (histoTH1D != nullptr) histoTH1D->SetAxisColor(kRed);
  // ############################################
