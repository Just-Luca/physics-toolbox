/*
c++ -o Likelihood_vs_Chi2 Likelihood_vs_Chi2.cpp `root-config --cflags --glibs`
*/
...
void computeChi2 (TH1D& histo, double mean, const char* fitType)
{
...
int main (int argc, char** argv)
{
  ...
  if (argc < 2)
    {
      ...
      cout << "./Likelihood_vs_Chi2 [datiL.txt/datiS.txt]" << endl;
      return 1;
    }

  TCanvas c1("c1","My Canvas");
  c1.Divide(2,1);
  ...
  int nbin    = max - min;
  double myrand;
  TH1D histoMin2("histoMin2","histoMin2",nbin,min,max);
  TH1D histoLike("histoLike","histoLike",nbin,min,max);
  ...
  // # Likelihood fit #
  // ##################
  TFitResultPtr resultLike = histoLike.Fit("pol0","S L");
  meanLike = funzione.GetParameter(0);
  errLike  = funzione.GetParError(0);
  chiLike  = 2. * resultLike->MinFcnValue(); // Correct way to compute inference with a likelihood fit
  ...
  computeChi2(histoMin2, meanMin2, "Min2");
  cout << "\nCHIQUADRO FIT OPZIONE Likelihood " << endl;
  computeChi2(histoLike, meanLike, "Like");
