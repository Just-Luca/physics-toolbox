/*
c++ -o esercizio05 esercizio05.cpp myLib.cc `root-config --glibs --cflags`
*/
...
int main()
{
  ...
  for (unsigned int i = 0; i < N; i++)
    {
      valNew = Statistics::genLinCong(valOld);
      // std::cout << "Generated number [" << i << "] = " << valNew << std::endl;
      valOld = valNew;

      h0.Fill(static_cast<double>(valNew) / Statistics::GENLINCONG::M);
    }


  double mean  = 0.5;             // Mean value of the uniform distribution in the unterval [0,1)
  double sigma = 1/sqrt(12. * Q); // Standard deviation of the uniform distribution in the interval [0,1) divided by sqrt(Q),
  ...
  for (unsigned int i = 0; i < N/100; i++)
    {
      ...
      for (unsigned int j = 0; j < Q; j++)
        {
          valNew = Statistics::genLinCong(valOld);
          avg   += static_cast<double>(valNew) / Statistics::GENLINCONG::M;
          valOld = valNew;
        }
      avg /= Q;
  ...
  // #############
  c0.cd();
  h0.GetYaxis()->SetRangeUser(0,N/NBINS + N/NBINS);
  h0.Draw();
